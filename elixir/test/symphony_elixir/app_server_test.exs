defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server marks MCP elicitation requests as a hard failure" do
    test_root =
      Path.join(
        System.tmp_dir!(),
    ...
    try do
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "MT-1088")
      codex_binary = Path.join(test_root, "fake-codex")
      File.mkdir_p!(workspace)
      ...
      while IFS= read -r _line; do
        ...
        case "$count" in
          1)
            ...
            ;;
          2)
            printf '%s\\n' '{\"id\":2,\"result\":{\"thread\":{\"id\":\"thread-1088\"}}}'
            ;;
          3)
            printf '%s\\n' '{\"id\":3,\"result\":{\"turn\":{\"id\":\"turn-1088\"}}}'
            ;;
          4)
            printf '%s\\n' '{\"method\":\"mcpServer/elicitation/request\",\"id\":\"mcp-1\",\"params\":{\"server\":\"codex_apps\",\"tool\":\"linear\",\"request\":\"Choose an issue source\"}}'
            ;;
          *)
            exit 0
            ;;
      ...
      issue = %Issue{
        ...
        identifier: "MT-1088",
        title: "MCP elicitation needed",
        description: "Cannot satisfy mcp elicitation",
        state: "In Progress",
        url: "https://example.org/issues/MT-1088",
        labels: ["backend"]
      }

      assert {:error, {:turn_input_required, payload}} =
               AppServer.run(workspace, "Needs MCP elicitation", issue)
